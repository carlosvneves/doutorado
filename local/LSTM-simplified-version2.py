...
X_train = data_train[['Agr','Ind','Inf','Com']]
...
X_train, y_train = create_dataset(X_train, y_train,n_qtr)
X_train, y_train = np.array(X_train), np.array(y_train)
...
# 
# X_val, y_val = create_dataset(X_val, y_val,n_qtr)
# X_val, y_val = np.array(X_val), np.array(y_val)
...
X_test = data_test[['Agr','Ind','Inf','Com']]
...
dates_forecast = pd.date_range(start='2020-01-01', periods=n_ahead, freq='Q')
...
X_forecast = df_forecast[['Agr','Ind','Inf','Com']]
...
X_forecast, y_forecast  = create_dataset(X_forecast, y_forecast,nqtr)
X_forecast, y_forecast = np.array(X_forecast), np.array(y_forecast)

#

# Make prediction
yhat = reg.predict(X_forecast)
